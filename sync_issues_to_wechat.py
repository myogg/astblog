...
class GitHubIssueSyncer:
    ...
    def sync(self):
        """执行同步操作"""
        issues = self.get_issues_to_sync()
        
        if not issues:
            print("没有需要同步的 Issues")
            return
        
        print(f"找到 {len(issues)} 个需要同步的 Issues")
        
        for issue in issues:
            try:
                # 解析 Issue
                article_data = self.parse_issue(issue)
...
def main():
    ...
    config = {
        ...
        'sync_label': os.getenv('SYNC_LABEL', 'wechat-publish'),
    ...
    # 验证必要配置
    if not all([config['github_token'], config['wechat_appid'], config['wechat_secret']]):
        print("错误: 缺少必要的配置")
        print("请设置环境变量: GITHUB_TOKEN, WECHAT_APPID, WECHAT_SECRET")
        return
