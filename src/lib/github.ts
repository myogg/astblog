...
export interface Label {
	name: string;
	color: string; // GitHub's 6-character hex color without #
	description?: string;
}
...
export interface BlogPost {
	...
	labels: Label[]; // Label list with colors
				...
				return {
					...
					labels: issue.labels.map((label) => {
						if (typeof label === "string") {
							return { name: label, color: "0366d6" }; // Default GitHub blue
						}
						return {
							name: label.name || "",
							color: label.color || "0366d6",
